...
if (!$board['bo_use_list_view']) {
    ...
    // 윗글을 얻음
    $sql = " select wr_id, wr_subject from {$write_table} where wr_is_comment = 0 and mb_id ='{$member['mb_id']}' and wr_num = '{$write['wr_num']}' and wr_reply < '{$write['wr_reply']}' {$sql_search} order by wr_num desc, wr_reply desc limit 1 ";
    $prev = sql_fetch($sql);
    // 위의 쿼리문으로 값을 얻지 못했다면
    if (!$prev['wr_id'])     {
        $sql = " select wr_id, wr_subject from {$write_table} where wr_is_comment = 0 and mb_id ='{$member['mb_id']}' and wr_num < '{$write['wr_num']}' {$sql_search} order by wr_num desc, wr_reply desc limit 1 ";
        $prev = sql_fetch($sql);
    }

    // 아래글을 얻음
    $sql = " select wr_id, wr_subject from {$write_table} where wr_is_comment = 0 and mb_id ='{$member['mb_id']}' and wr_num = '{$write['wr_num']}' and wr_reply > '{$write['wr_reply']}' {$sql_search} order by wr_num, wr_reply limit 1 ";
    $next = sql_fetch($sql);
    // 위의 쿼리문으로 값을 얻지 못했다면
    if (!$next['wr_id']) {
        $sql = " select wr_id, wr_subject from {$write_table} where wr_is_comment = 0 and mb_id ='{$member['mb_id']}' and wr_num > '{$write['wr_num']}' {$sql_search} order by wr_num, wr_reply limit 1 ";
        $next = sql_fetch($sql);
    }
}
...
if ($is_member) {
	...
	$qtype=$write['wr_5'];
	$write2 = sql_fetch(" select w.*,q.wr_1 as q_1
			,q.wr_2 as q_2,q.wr_3 as q_3,q.wr_4 as q_4,q.wr_5 as q_5,q.wr_6 as q_6,q.wr_7 as q_7
			,q.wr_8 as q_8,q.wr_9 as q_9,q.wr_10 as q_10,q.wr_11 as q_11,q.wr_12 as q_12,q.wr_13 as q_13
			,q.wr_14 as q_14,q.wr_15 as q_15,q.wr_id as q_id from $write_table w inner join g5_write_{$qtype} q on w.wr_2 = q.wr_id where w.wr_id = '$wr_id' ");

$view = get_qview($write2, $board, $board_skin_path);
