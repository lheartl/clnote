...
// 게시물 정보($write_row)를 출력하기 위하여 $list로 가공된 정보를 복사 및 가공
function get_qlist($write_row, $board, $skin_url, $subject_len=40)
{
	global $g5, $config;
	global $qstr, $page;
	...
	$qhtml.="문제.".$list['q_id']." ".$list['q_1'];
	$qhtml.=" <font color='red'>정답 :".$list['q_14']."</font>";
	$qhtml.="</th></tr></thead><tbody><tr><td class='font-size-12'>";
	if($list['q_2']!=''){//문제 이미지1
		$qhtml.='<img src="'.G5_PATH.'/hbqImg/'.$list['q_2'].'">';
	}
	if($list['q_3']!=''){//문제 이미지2
		$qhtml.='<br><img src="'.G5_PATH.'/hbqImg/'.$list['q_3'].'">';
	}
	$qhtml.='<div>';
	if($list['q_4']!=''){//문제 텍스트1
		$qhtml.=$list['q_4'];
	...
	if($list['q_9']!=''){//문제 보기1
		$qhtml.='<button class="eb-button eb-button-large eb-button-gray" style="text-align: left;" name="btn_submit" value="" onclick="chkDab(1);">'.$list['q_9'].'</button>';
	}
	if($list['q_10']!=''){//문제 보기2
		$qhtml.='<br><button class="eb-button eb-button-large eb-button-gray" style="text-align: left;" name="btn_submit" value="" onclick="chkDab(2);">'.$list['q_10'].'</button>';
	}
	if($list['q_11']!=''){//문제 보기3
		$qhtml.='<br><button class="eb-button eb-button-large eb-button-gray" style="text-align: left;" name="btn_submit" value="" onclick="chkDab(3);">'.$list['q_11'].'</button>';
	}
	if($list['q_12']!=''){//문제 보기4
		$qhtml.='<br><button class="eb-button eb-button-large eb-button-gray" style="text-align: left;" name="btn_submit" value="" onclick="chkDab(4);">'.$list['q_12'].'</button>';
	}
	if($list['q_13']!=''){//문제 보기5
		$qhtml.='<br><button class="eb-button eb-button-large eb-button-gray" style="text-align: left;" name="btn_submit" value="" onclick="chkDab(5);">'.$list['q_13'].'</button>';
	}
...
// get_list 의 alias
function get_qview($write_row, $board, $skin_url)
{
	return get_qlist($write_row, $board, $skin_url, 255);
} 
